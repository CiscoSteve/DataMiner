                ...
                if response.status_code == 200:
                    ...
                    if debug_level == 1 or debug_level == 2:
                        print("\nTotal Pages:", pages,
                              ...
                              "\n====================")
                    page = 0
                    print(f"Found details contract number {contractNumber} for {customerName}")
                    while page < pages:
        ...
        if items is not None:
            if len(items) > 0:
                for item in items:
                    ...
                    createdTime = str(item['created'])
                    createdTime = int(createdTime) / 1000
                    createdOn = (datetime.utcfromtimestamp(createdTime).strftime('%Y-%m-%d %H:%M:%S'))
                    modifiedBy = item['offerType'].replace(",", " ")
                    modifiedTime = str(item['modified'])
                    modifiedTime = int(modifiedTime) / 1000
                    modifiedOn = (datetime.utcfromtimestamp(modifiedTime).strftime('%Y-%m-%d %H:%M:%S'))
                            ...
                            for row in checklists:
                                ...
                                with open(partnerOffers, 'a', encoding="utf-8", newline='') as target:
                                    writer = csv.writer(target,
                                                        ...
                                                        quotechar=' ',
                                                        quoting=csv.QUOTE_MINIMAL)
                                    print(f"Found Partner Offers for customer {companyName}")
                                    CSV_Data = (customerId + ',' +
                                                offerId + ',' +
                                                offerType + ',' +
                                                title + ',' +
...
def pxc_assets_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Asset data for Customer:", customerId,
            ...
            if not successTrackId == "N/A":
                ...
                with open(input_json_file, 'r', encoding='utf-8') as inputTarget:
                    ...
                    items = data['items']
                if items is not None:
                    print(f"Found Asset Data on Success Track {successTrackId} for customer {customerId}")
                    if len(items) > 0:
                        for item in items:
                    ...
                    print("\nNo Success Track Data Found For Customer:", customerId,
                          "\nSkipping this record and continuing....\n", end="")
                if outputFormat == 1 or outputFormat == 2:
                    print(f"Saving JSON file for customer {customerId} on successtrack {successTrackId}")
...
def pxc_hardware_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Hardware data for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
                ...
                try:
                    ...
                    print("\n!!!!!!!!\nException Thrown for", "Customer:", customerId,
                          "on Success Tracks:", successTrackId,
                          "Report Failed to Download\n")
                    print("Report request URL:", url,
                          "\nReport details:", data_payload,
                    ...
                    while response.status_code >= 400:
                        ...
                        print("Report request URL:", url,
                              ...
                              "\nReview API Headers:", response.headers,
                              "\nResponse Body:", response.content)
                        print(f"Review for Customer: {customerId} on Success Tracks:{successTrackId} "
                ...
                with open(input_json_file, 'r', encoding='utf-8') as inputTarget:
                    ...
                    items = data['items']
                if not items == []:
                    print(f"Found Hardware Data on Success Track {successTrackId} for customer {customerId}")
                    for item in items:
                        assetId = str(item['assetId']).replace(",", "_")
                    ...
                    print("\nNo Data Found .... Skipping for Customer:", customerId,
                          "on Success Tracks:", successTrackId,
                          "\nSkipping this record and continuing....\n", end="")
                if outputFormat == 1 or outputFormat == 2:
                    print(f"Saving JSON file for customer {customerId} on successtrack {successTrackId}")
...
def pxc_software_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Software data for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
                ...
                with open(input_json_file, 'r', encoding='utf-8') as inputTarget:
                    ...
                    items = data['items']
                if not items == []:
                    print(f"Found Software Data on Success Track {successTrackId} for customer {customerId}")
                    for item in items:
                        assetName = str(item['assetName'])
                        assetId = str(item['assetId']).replace(",", "_")
                        productId = str(item['productId'])
...
def pxc_purchased_licenses_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Purchased Licenses data for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
                ...
                with open(input_json_file, 'r', encoding='utf-8') as inputTarget:
                    ...
                    items = data['items']
                if not items == []:
                    print(f"Found Purchased Licenses Data on Success Track {successTrackId} for customer {customerId}")
                    for item in items:
...
def pxc_licenses_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("License data for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
                ...
                with open(input_json_file, 'r', encoding='utf-8') as inputTarget:
                    ...
                    items = data['items']
                if not items == []:
                    print(f"Found License Data on Success Track {successTrackId} for customer {customerId}")
                    for item in items:
                        assetId = str(item['assetId']).replace(",", "_")
...
def get_pxc_lifecycle():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Lifecycle data for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                ...
                if outputFormat == 1 or outputFormat == 2:
                    if items is not None:
                        if len(items) > 1:
                            with open(json_output_dir + customerId + "_Lifecycle.json",
                                      'w') as json_file:
                                json.dump(items, json_file)
                            print(f"Saving {json_file.name}")
                if items is not None:
                            ...
                            for useCase in useCases:
                                ...
                                for pitstop in pitstops:
                                    ...
                                    for pitstopAction in pitstopActions:
                                        ...
                                        if outputFormat == 1 or outputFormat == 3:
                                            with open(lifecycle, 'a', encoding="utf-8", newline='') as lifecycle_target:
...
def pxc_security_advisories_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Security Advisories for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
...
def pxc_field_notices_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Field Notices for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
...
def pxc_priority_bugs_reports():
    ...
    with open(customers, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if debug_level == 1 or debug_level == 2:
                print("====================\n\n")
                print("Priority Bugs for Customer:", customerId, "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                url = (pxc_url_customers + "/" + customerId + "/reports")
    ...
    with open(SWGroups, 'r') as target:
        ...
        for row in csvreader:
            ...
            if not successTrackId == "N/A":
                ...
                print(f"\nSaving Software Group Risk Trend to CSV")
        ...
        for row in csvreader:
            ...
            if debug_level == 1 or debug_level == 2:
                ...
                print("Software Group Suggestions Security Advisories for Customer:", customerId,
                      "on Success Tracks:", successTrackId)
            if not successTrackId == "N/A":
                print(f"Found Software Group Suggestions Security Advisory for {customerName} "
                      f"with machine suggestion ID of {machineSuggestionId}")
                url = (pxc_url_customers + "/" +
                       customerId +
...
def pxc_automated_fault_management_faults():
    ...
    with open(customers, 'r') as cust_target:
        custList = csv.DictReader(cust_target)
        for row in custList:
            print("Automated Fault Management Faults for Customer:", row["customerId"],
                ...
                if response.status_code == 200:
                    ...
                    while page < pages:
                        ...
                        if outputFormat == 1 or outputFormat == 3:
                            if items is not None:
                                if len(items) > 0:
                                    print(f"Found Compliance Violations on Success Track {successTrackId} "
                                          f"for {customerName}")
                                    for item in items:
                                        severity = item['severity'].replace(",", " ")
                                        ...
                                        with open(RCCComplianceViolations, 'a', encoding="utf-8", newline='') as target:
                                            writer = csv.writer(target,
                                                                delimiter=' ',
                                                                quotechar=' ',
                                                                quoting=csv.QUOTE_MINIMAL)
            ...
            if not row["successTrackId"] == "N/A":
                ...
                if outputFormat == 1 or outputFormat == 2:
                    if items is not None:
                        if len(items) > 1:
                            with open(
                                    json_output_dir + customerId + "_Assets_with_Violations_" +
                                    successTrackId + ".json", 'w') as json_file:
                                json.dump(items, json_file)
                            print(f"Saving {json_file.name}")
                if outputFormat == 1 or outputFormat == 3:
                    print(f"Found Assets with Violations on Success Track {successTrackId} "
                          f"for {customerName}")
                    if items is not None:
                        if len(items) > 0:
                            for item in items:
...
def pxc_asset_violations():
    ...
    with open(RCCAssetsWithViolations, 'r') as cust_target:
        ...
        for row in custList:
            ...
            try:
                if outputFormat == 1 or outputFormat == 3:
                    print(f"Found Assets Violations on Success Track {successTrackId} "
                          f"for {customerName}")
                    if items is not None:
                        if len(items) > 0:
                            for item in items:
                ...
                if outputFormat == 1 or outputFormat == 2:
                    if items is not None:
                        if len(items) > 1:
                            ...
                            print(f"Saving {json_file.name}")
                    if outputFormat == 1 or outputFormat == 3:
                        print(f"Found Regulatory Compliance Obtained on Success Track {successTrackId} "
                              f"for {customerName}")
                ...
                if items is not None:
                    if len(items) > 1:
                        if outputFormat == 1 or outputFormat == 2:
                            ...
                            print(f"Saving {json_file.name}")
                        if outputFormat == 1 or outputFormat == 3:
                            print(f"Found Risk Mitigation Checks on Crash Risk Assets on Success Track {successTrackId}"
                                  f" for {customerName}")
                            if items:
                                for item in items:
    ...
    with open(CrashRiskAssets, 'r') as cust_target:
        ...
        for row in custList:
            ...
            if not row["successTrackId"] == "N/A":
                ...
                if debug_level == 1 or debug_level == 2:
                    print(url)
                print(f"Found Crash Risk Factors for asset ID {assetId}"
                      f" for {customerName}")
                items = get_json_reply(url, tag="items")
                if outputFormat == 1 or outputFormat == 2:
                    if items is not None:
        ...
        for row in custList:
            ...
            if not row["successTrackId"] == "N/A":
                for feature in features:
                    ...
                    print(f"Found Similar Asset for asset ID {assetId} for {customerName} with same {feature}")
                    items = get_json_reply(url, tag="items")
                    crashPredicted = str(get_json_reply(url, tag="crashPredicted"))
                    if outputFormat == 1 or outputFormat == 2:
                        if items is not None:
                    ...
                    if outputFormat == 1 or outputFormat == 2:
                        if items is not None:
                            if len(items) > 1:
                                ...
                                print(f"Saving {json_file.name}")
                    if outputFormat == 1 or outputFormat == 3:
                        print(f"Found Assets Last Crashed in {daysLastCrashed} days for {customerName} "
                              f"on Success Tracks: {successTrackId}")
                        if items:
                            for item in items:
                                assetId = str(item['assetId']).replace(",", "_")
                                assetName = str(item['assetName']).replace(",", " ")
        ...
        for row in custList:
            ...
            if not row["successTrackId"] == "N/A":
                url = (pxc_url_customers + "/" +
                       ...
                       "?successTrackId=" + successTrackId +
                       "&max=" + max_items)
                print(f"Found Risk Mitigation Checks on Assets Crash History for {customerName} "
                      f"with Asset {assetId}")
                items = get_json_reply(url, tag="items")
                if outputFormat == 1 or outputFormat == 2:
                    if items is not None:
                        if len(items) > 1:
...
for x in range(0, testLoop):
    if debug_level == 0:
        ...
        logLevel = "High"
    if log_to_file == "True" or log_to_file == "true":
        print("Logging output to file PXCloud_##.log with debug level ", logLevel, "for version ", codeVersion)
    ...
    # Record time and cleanly
    # , or run next itteration
    print("test run", x + 1, " Completed Successfully")
    stopTime = time.time()
